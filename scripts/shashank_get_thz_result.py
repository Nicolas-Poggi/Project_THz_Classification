...
def get_output_and_setup_model(pipeline,model_name, system_prompt, max_tokens):
    """
    Setup the model with the system prompt.
    """
    model_output = ""
    # Load the model and processor based on the selected model
    if model_name == "mistral":
        model_output = pipeline.setup_mistral(prompt=system_prompt, max_tokens=max_tokens)

    elif model_name == "qwen":
        model_output = pipeline.setup_qwen(prompt=system_prompt, max_tokens=max_tokens)    
    
    elif model_name == "llama4":
        model_output = pipeline.setup_llama4(prompt=system_prompt, max_tokens=max_tokens)
    
    return model_output

def get_model_processor(model_name, model_path):

    if model_name == "mistral":
        from models.mistral import MistralScores
        processor = AutoProcessor.from_pretrained(model_path)
        model = AutoModelForImageTextToText.from_pretrained(model_path, device_map="cuda", torch_dtype=torch.bfloat16)
        ...
        model = AutoModelForImageTextToText.from_pretrained(model_path, device_map="cuda", torch_dtype="bfloat16")
    ...
    #     processor = AutoProcessor.from_pretrained(model_path , device_map='cuda', torch_dtype='bfloat16', trust_remote_code=True)
    #     model = AutoModelForCausalLM.from_pretrained(model_path, trust_remote_code=True, torch_dtype='bfloat16', device_map='cuda')
    #     pipeline = MolmoScores(model=model, processor=processor)

    else:
        raise ValueError(
            "Invalid model name. Choose from 'mistral', 'qwen', or 'llama4'."
            )
...
def get_model_answer(pipeline, model_name, max_tokens, text, image_path):
    model_output = ""
    if model_name == "mistral":
         model_output = pipeline.get_answer(prompt=text, image_path=image_path, max_tokens=max_tokens)
...
def main(args):
    ...
    #--------------------------------------------------------------------------------------------------
    # Models - When changing the [modelpath] also change [test_addon] fittingly in parameters for the output file
    #
    ...
    output_folder_filepath = "/pfs/work9/workspace/scratch/ma_npoggigo-bachelor_thesis_fss2025/Project_THz_Classification/experiments/history_test/shashank_results"
    test_addon = "qwen"
    ...
    if args.image_folder_filepath != None: 
        image_folder_filepath = args.image_folder_filepath
    ...
    images_paths = get_image_paths(image_folder_filepath)
    ...
    pipeline = get_model_processor(model_name, model_path)
    ...
    #
    # "setup" the model with the System Prompt (input prompt: only text)
    # Append the system Prompt to the output file
    # Print the system Prompt - simple format
    # Append the model output to the output file
    # Print the model output - simple format
    # increment the counter
    #--------------------------------------------------------------------------------------------------
    model_output = get_output_and_setup_model(pipeline=pipeline, model_name=model_name, system_prompt=system_prompt, max_tokens=max_tokens)
    ...
    if(should_test_history == True):
        ...
        model_output = get_model_answer(pipeline=pipeline, model_name=model_name, max_tokens=max_tokens, text=test_frame_prompt_one, image_path=test_image_filepath)
    ...
    for image_filepath in images_paths:
        ...
        with torch.inference_mode():
            model_output = pipeline.get_answer(image_path=image_filepath, prompt=edited_frame_prompt, max_tokens=max_tokens)      
...
if __name__ == "__main__":
    ...
    parser.add_argument("--image_folder_filepath", type=str, required=False, 
